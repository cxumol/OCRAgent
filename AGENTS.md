...
- 先检查各类配置/所需文件 (环境变量, user toolbox txt/py 是否齐备), 如果不足以仅凭 ocragent 各子命令完成全流程, 则提醒缺少的文件
> 比如, 缺少 user toolbox txt, 但 user toolbox py 已完整可用, 则可从 init docs 开始执行; 缺少 user toolbox py 但有 txt, 则从 init tools 开始执行; 但同时缺少两者, 则需要引导用户提供 user toolbox txt 并告诉用户在提供相应 txt / 配好环境变量后重新启动 OCRAgent
- 顺序执行 ocragent 各流程子命令; 并在底部显示当前阶段和进度
- 意外退出时询问用户是否需要重试

此为大致粗略思路, 具体见 docs/auto-mode.md
...
logo: 天平两端分别摆放文件 (羊皮卷、竹简、U 盘) 和 金银铜币
横幅: title+logo+slogan (agent-guided OCR.\n何须一模破万卷，智能调度在慧枢。)
对比式示意图: 左右对比, 左边对应 "Single OCR Model", 右边对应 "Agentic OCR Router", 少画单词, 多采用形象思维示意, 文本语言为英文
详细架构图: 以不同视觉通道编码 人机/机机交互, 内置/扩展能力, 程序性逻辑/AgenticLoop, 通用/AI算力 这几个正交维度, 文本语言为英文
